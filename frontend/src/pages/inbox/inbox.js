...
import {
  ...
  sendMessage,
  deleteMessages,
} from "../../features/chatSlice"; // Ensure correct path
...
export default function Inbox({ userId }) {
  ...
  const { chatId, combineInfo, loading, error } = useSelector(
    (state) => state.inbox
  );
            ...
            {updatedCombine.length > 0 ? (
              <div>
                {updatedCombine.map((prod) => (
                  <div
                    onClick={() => handleViewChat(prod.chatId, prod.sellerId)}
                  ...
                  >
                    ...
                    <button onClick={() => handleDelete(prod.chatId)}>
                      {" "}
